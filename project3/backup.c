...
#define TIMELENGTH 200
#define MAXFORMATSIZE 250
#define SOURCE "-s"
...
int main(int argc, char **argv) {
	...
	char *oldLogFilePath = malloc(sizeof(char)*strlen(destDir) + /*open old log file, if it exists*/
...
void putFStats(char *fileName, char **buf) {
	...
	strncpy(fmTime, mTime,strlen(mTime));
	fmTime[strlen(mTime)] = '\0';
  ...
  int subWasMalloced = 0;
	...
	while(n--) {
		...
		if (S_ISDIR(tempStats->st_mode)) { /* if dir mkdir in backupDir, and copyDir on it*/ 
			...
			subDirSource = malloc(sizeof(sourceDir) +
					sizeof(dirList[n]->d_name) +
					2);
      subWasMalloced = 1;
			sprintf(subDirSource,"%s/%s",sourceDir,dirList[n]->d_name);
